...
class CalculatorViewModel : ViewModel() {
    ...
    private fun performDeletion() {
        when {
            ...
            state.operation == null -> state = state.copy(number1 = state.number1.dropLast(1))
        }
    }
    ...
    private fun enterNumber(n: Int) {
        if (state.operation == null) {
            ...
            state = state.copy(number1 = state.number1 + n)
            return
        }
        if (state.number2.length >= MAX_NUM_LENGTH) {
            return
        }
        state = state.copy(number2 = state.number2 + n)
        return
    }
